...
pub enum Commands {
	...
	Update(Update),
	Upgrade(Upgrade),
	/// Upgrade packages, allowing removals to resolve dependency changes.
	#[clap(name = "full-upgrade", alias = "dist-upgrade")]
	FullUpgrade(Upgrade),
	/// Upgrade packages without installing or removing packages.
	#[clap(name = "safe-upgrade")]
	SafeUpgrade(Upgrade),
	Install(Install),
...
pub struct Search {
	...
	#[clap(short = 'n', long = "names", visible_alias = "names-only", action)]
	pub names_only: bool,
...
pub struct Clean {
	...
	/// Removes the `nala.sources` file generated by the fetch command
	#[clap(long, action)]
	pub fetch: bool,
}
