...
impl HistoryEntry {
	/// Marks the inverse of this entry's replayable package rows and commits them.
	pub async fn undo(&self, config: &mut Config) -> Result<()> {
		let cache = self.prepare_replay(config)?;

		for package in self.replayable() {
	...
	/// Replays this entry's supported package rows into the cache and commits them.
	...
	fn prepare_replay(&self, config: &mut Config) -> Result<Cache> {
		...
		if self.replayable().next().is_none() {
